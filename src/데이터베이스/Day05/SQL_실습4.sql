...
select count(*) 총주문건수 from orders;

-- [문제 2] books 테이블에 있는 모든 도서의 평균 가격(price)을 '평균단가' 라는 별칭으로 조회하세요.
select avg(price) 평균단가 from books;

-- [문제 3] orders 테이블에서 '김민준' 고객이 주문한 총 주문 수량(order_qty의 합계)을 조회하세요.
select sum(order_qty) 총주문수량 from orders where customer = '김민준';

-- [문제 4] books 테이블에서 각 장르(genre)별로 도서가 총 몇 권씩 있는지 조회하세요.
select genre, count(*) 총개수 from books group by genre;

-- [문제 5] orders 테이블에서 각 도서번호(book_id)별로 총 몇 권이 주문되었는지 조회하세요.
select book_id 도서번호, sum(order_qty) 총개수 from orders group by book_id;

-- [문제 6] '김민준'과 '이서연' 고객에 대해, 각 고객별 총 주문 수량(order_qty의 합계)을 조회하세요. 단, 총 주문 수량이 4권 이상인 고객만 조회하세요.
select customer 고객명, sum(order_qty) 총주문수량 from orders 
	where customer in ('김민준', '이서연') group by customer having 총주문수량 >= 4;

-- [문제 7] books 테이블에서 재고(stock)가 5권 미만인 도서들의 평균 가격을 조회하세요.
select * from books;
select * from books where stock < 5;					# 재고가 5권 미만인 도서들
select avg(price) 평균가격 from books where stock < 5;	# 의 평균 가격

-- [문제 8] orders 테이블에서 3번 이상 주문된 도서의 book_id와 주문 횟수를 조회하세요.
select * from orders;
select book_id, count(*) 주문횟수 from orders group by book_id;							# book_id별 주문횟수 조회
select book_id, count(*) 주문횟수 from orders group by book_id having 주문횟수 >= 3;			# 3번 이상 주문 조건

-- [문제 9] orders 테이블에서 총 주문 수량이 5권을 초과하는 고객의 customer와 총 주문 수량을 조회하세요.
select * from orders;
select customer 고객명, sum(order_qty) 총주문수량 from orders group by customer; 						# 고객별 총주문수량 파악하기
select customer 고객명, sum(order_qty) 총주문수량 from orders group by customer having 총주문수량 > 5;	# 조건 추가

-- [문제 10] books 테이블에서 각 저자(author)별로 출판한 도서가 2권 이상인 저자의 이름과 출판 도서 수를 조회하세요.
select * from books;
select author 저자명, count(*) 출판도서수 from books group by author having 출판도서수 >= 2;

-- [문제 11] books 테이블의 '컴퓨터' 장르 도서 중 가장 비싼 도서의 가격을 조회하세요.
select max(price) from books where genre = '컴퓨터' group by genre;

-- [문제 12] books 테이블의 '소설' 장르 도서들의 총 재고 수량을 조회하세요.
select genre 장르, sum(stock) 총재고수량 from books where genre = '소설';

-- [문제 13] orders 테이블에서 주문을 한 고객이 총 몇 명인지 조회하세요. (DISTINCT 사용)
select count(distinct customer) 주문고객수 from orders;

-- [문제 14] orders 테이블에서 고객별로 첫 주문일과 마지막 주문일을 조회하세요.
select customer 고객명, min(order_date) 첫주문일, max(order_date) 마지막주문일
...
select genre 장르, avg(price) 평균가격 from books group by genre having 평균가격 > 18000;

-- [문제 17] orders 테이블에서 2023년 1분기(1월~3월)에 발생한 총 주문 수량을 조회하세요. (BETWEEN 사용)
select sum(order_qty) 총주문수량 from orders where order_date between '2023-01-01' and '2023-03-31';

-- [문제 18] orders 테이블에서 가장 다양한 종류의 책을 주문한 고객의 이름과 그 종류의 수를 조회하세요.
select * from orders;
select customer 고객명, book_id from orders order by customer, book_id;
select customer 고객명, count(distinct book_id) 종류수 from orders group by customer;
select customer 고객명, count(distinct book_id) 종류수 from orders group by customer order by 종류수 desc limit 1;


-- [문제 19] books 테이블에서 각 장르별로 가장 저렴한 도서의 가격을 조회하세요.
select * from books;
select genre 장르, min(price) 최소가격 from books group by 장르;
